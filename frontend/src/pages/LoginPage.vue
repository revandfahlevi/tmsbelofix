    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-2xl shadow-xl border border-blue-50 p-8">
        ...
        <div v-if="googleError"
          class="bg-red-50 border border-red-200 rounded-xl p-3 mb-4 flex items-center gap-2">
          <AlertCircle class="w-4 h-4 text-red-500 flex-shrink-0" />
          <p class="text-sm text-red-600">{{ googleError }}</p>
        </div>
        ...
        <button
          ...
          class="w-full flex items-center justify-center gap-3 border border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed py-2.5 rounded-xl text-sm font-medium text-gray-700 transition">
          <!-- Spinner saat loading Google -->
...
// Cek kalau ada error dari Google OAuth di URL (?error=...)
onMounted(() => {
  const err = route.query.error as string
  if (err === 'google_failed') {
    googleError.value = 'Login dengan Google gagal. Silakan coba lagi.'
  } else if (err === 'account_suspended') {
    googleError.value = 'Akun Anda telah dinonaktifkan. Hubungi administrator.'
  }
})
...
function handleGoogleLogin() {
  ...
  // Redirect ke Laravel — Laravel yang handle OAuth ke Google
